...
## Reporting, matching & source-legality layer (workorder BLOK 1-4, 2026-06-07)
Added by the workorder (`WORKORDER.md` lives on the Backup USB, not in the repo). All migrations applied to the remote DB; code on branches `workorder-bloks-1-4` + `bugfix-audit-round1` (not yet merged to main — merge the bugfix branch, it carries both). Full detail in `STATUS.md` §3c-3f + §4b and `operations/blok4-crawler.md`.
- **BLOK 1 — `reports` analytics.** Analytical fields are typed/indexed columns (`country`, `report_type` enum buyer|farmer, `risk_reduction`, `justified_farm_gate_price`, `buyer_location`) next to the raw `input_data`/`report_data` JSON. Cross-vocabulary linkage: `orbag_canonical_crop()` bridges the farmer Dutch crop keys (`wintertarwe`) and buyer English keys (`wheat`); `match_key` (generated col) + the `report_match_keys` view (explodes multi-crop farmer rows) join a buyer report to a farmer report over **canonical-crop + country** (NOT the literal `crop:region`, which never links). `save_report` RPC writes the new columns.
- **BLOK 3 — country registry.** `intelligence_countries` extended (`iso_code`/`currency`/`region_cluster`/`benchmark_source_planned`/`engine_region_key`), seeded EU-27 + UK + MA/EG/TN, `rotation_active=false`. **Fail-loud:** no placeholder benchmark rows (would fabricate a number); the `country_benchmark_status` view surfaces "no benchmark available — do NOT fall back" for any country without its own data.
- **BLOK 4 — source legality.** `intelligence_sources` (green/orange/red `legality_class`, `scraping_allowed` default false) with a **hard DB invariant**: `scraping_allowed=true` only with an explicit licence and a non-RED class. `crawler_source_queue` view = the crawler's allowed daily worklist (API→RSS→scrape). The n8n crawler writes via `upsert_intelligence_signal()` (never a direct POST). Adding/allowing a NEW source is Leo-gated.
- **BLOK 2 — match scoring (gated).** `buyer_bcs`/`farmer_bcs` gained the niveau-1/2 fields; `api/match-economics.js` computes logistics cost + net-value-per-tonne for one farmer⇄buyer pair (customs duty keyed per source country, mirrors `generate.js`). The **scoring weights are a business decision — not wired**; awaiting Leo. ⚠ The match page is currently **empty for everyone** because `buyer_bcs`/`profiles` RLS is `auth.uid()=id` (no cross-user reads) — opening it is a privacy decision (see `OPEN-VOOR-JOU.md`).
- **Tests:** `node tests/match.test.js` (match economics) alongside engine/farmer/concentration. Run all four before committing engine/match changes.
